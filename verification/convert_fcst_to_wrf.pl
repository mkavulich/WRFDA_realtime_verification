...
# Dates (should convert this to allow command-line input in future
 my $start_date="2017-02-20_00:00:00"; #Initial time for first GFS forecast
 my $end_date="2017-02-27_00:00:00";   #Initial time for final GFS forecast
 my $FC_INTERVAL=24;                   #Interval between forecasts  IN HOURS

 my @convert_hours = ( 0, 48 );          #Forecast hours to convert for each individual forecast
...
# Directories for running WPS/WRF and storing output (NOTE: WILL BE STORED IN DATE SUBDIRECTORIES)
 my $Run_dir_top="$WORKDIR/Run";
 my $Out_dir_top="$WORKDIR/Output";
...
# Check there are no problem values
 ...
 while ( &wrf2num($init_date) <= &wrf2num($end_date) ) {
   ...
   # If old data exists, ask to overwrite
    ...
    print "  GFS Forecast Start Time : $init_date\n";

    unless ($run_wps == 0) {

       # Get WPS files
       $GRIB_dir[0]="/glade/p/rda/data/ds084.1/$iyear/$iyear$imonth$iday";
...
#       print NL " eta_levels               = $NL_ETA_LEVELS\n";
       ...
       if ($run_wps == 0) {
          ...
          symlink sprintf("$GRIB_dir[0]/gfs.0p25.$iyear$imonth$iday$ihour.f%03d.grib2", $convert_hour) , "grib_$fyear$fmonth$fday\_$fhour" or die "Cannot symlink ".sprintf("$GRIB_dir[0]/gfs.0p25.$iyear$imonth$iday$ihour.f%03d.grib2", $convert_hour)." to grib_$fyear$fmonth$fday\_$fhour: $!\n";
