...
# Basic static inputs
 ...
 my $WRFDA_dir="/glade/p/work/kavulich/V39/WRFDA_3DVAR_dmpar_intel16/";
 my @WPS_indata_dir;
 my $Convert_GFS="true";
 my $Convert_NAM="true";
...
# THIS DATA SHOULD TAKE COMMAND-LINE ARGUMENTS IN THE FUTURE!
 ...
 my @SOUTH_NORTH_GRID;
# my @SOUTH_NORTH_GRID = ( 325, 986 ); Set below since NAM must be different
...
# Job submission options
 ...
 my $NUM_PROCS_REAL = 4;
 my $NUM_PROCS = 36; 
 my $JOBQUEUE_WPS = "share";
 my $JOBQUEUE_REAL = "share";
 my $JOBQUEUE = "regular";
...
# Check there are no problem values
 if (($NUM_PROCS_WPS > 18) and ($JOBQUEUE_WPS eq "share"))
     { die "\nERROR ERROR ERROR\nShare queue has a max NUM_PROCS of 18\nYou specified NUM_PROCS_WPS = $NUM_PROCS_WPS\nERROR ERROR ERROR\n\n"};
 if (($NUM_PROCS_REAL > 18) and ($JOBQUEUE_REAL eq "share"))
     { die "\nERROR ERROR ERROR\nShare queue has a max NUM_PROCS of 18\nYou specified NUM_PROCS_REAL = $NUM_PROCS_REAL\nERROR ERROR ERROR\n\n"};
 if (($NUM_PROCS > 18) and ($JOBQUEUE eq "share")) 
 ...
 foreach my $fcst (@forecasts) {
    ...
    while ( &wrf2num($init_date) <= &wrf2num($end_date) ) {
       ...
       my $job_feedback = ""; #For getting feedback from qsub
...
# RUN WPS
             ...
             open FH, ">run_wps_$convert_hour.pbs.csh" or die "Can not open run_wps_$convert_hour.pbs.csh for writing: $! \n";
                print FH "#!/bin/csh\n";
                print FH "#\n";
                print FH "# PBS batch script\n";
                print FH "# Automatically generated by $0\n";
                print FH "#PBS -N WPS_${fcst}2WRF_${convert_hour}h\n";
                print FH "#PBS -q $JOBQUEUE_WPS\n";
                print FH "#PBS -j oe\n";
                print FH "#PBS -o run_wps_$convert_hour.output\n";
                print FH "#PBS -e run_wps_$convert_hour.error\n";
                print FH "#PBS -l walltime=00:30:00"."\n";
                print FH "#PBS -A $PROJECT"."\n";
                printf FH "#PBS -l select=1:ncpus=%d:mpiprocs=%d"."\n", $NUM_PROCS_WPS, $NUM_PROCS_WPS;
                ...
                print FH "   mpirun ./geogrid.exe\n";
                print FH "endif\n";
                print FH "./link_grib.csh grib_*\n";
                unless ($fcst eq "MPAS") {
                   print FH "./ungrib.exe\n";
                }
                print FH "mpirun ./metgrid.exe\n";
             ...
             if ($job_feedback eq "") {
                $job_feedback = ` qsub < run_wps_$convert_hour.pbs.csh `;
             } else {
                $job_feedback = ` qsub -W depend=afterany:$jobid < run_wps_$convert_hour.pbs.csh `;
             }
             print "$job_feedback\n";
             if ($job_feedback =~ m/(\d+\..*)/) {
                $jobid = $1;
          ...
          if ($run_real =~ /f/i) { 
             ...
             open FH, ">run_real_$convert_hour.pbs.csh" or die "Can not open run_real_$convert_hour.pbs.csh for writing: $! \n";
             print FH "#!/bin/csh\n";
             print FH "#\n";
             print FH "# PBS batch script\n";
             print FH "# Automatically generated by $0\n";
             print FH "#PBS -N REAL_${fcst}2WRF_${convert_hour}h\n";
             print FH "#PBS -q $JOBQUEUE_REAL\n";
             print FH "#PBS -j oe\n";
             print FH "#PBS -o run_real_$convert_hour.output\n";
             print FH "#PBS -e run_real_$convert_hour.error\n";
             print FH "#PBS -l walltime=00:10:00"."\n";
             print FH "#PBS -A $PROJECT"."\n";
             printf FH "#PBS -l select=1:ncpus=%d:mpiprocs=%d"."\n", $NUM_PROCS_REAL, $NUM_PROCS_REAL;
             print FH "\n"; #End of PBS commands; add newline for readability
             print FH "\\rm namelist.input\n";
             print FH "ln -sf namelist.input.$convert_hour namelist.input\n";
             print FH "mpirun ./real.exe\n";
             ...
             if ($job_feedback eq "") {
                $job_feedback = ` qsub < run_real_$convert_hour.pbs.csh `;
             } else {
                $job_feedback = ` qsub -W depend=afterany:$jobid < run_real_$convert_hour.pbs.csh `;
             }
             print "$job_feedback\n";
             if ($job_feedback =~ m/(\d+\..*)/) {
                $jobid = $1;
          ...
          if ($run_wrf =~ /f/i) {
             ...
             open FH, ">run_wrf.pbs.csh" or die "Can not open run_wrf.pbs.csh for writing: $! \n";
             print FH "#!/bin/csh\n";
             print FH "#\n";
             print FH "# PBS batch script\n";
             print FH "# Automatically generated by $0\n";
             print FH "#PBS -N WRF${fyear}${fmonth}${fday}${fhour}${fcst}2WRF\n";
             print FH "#PBS -q $JOBQUEUE\n";
             print FH "#PBS -j oe\n";
             print FH "#PBS -o run_wrf_$convert_hour.output\n";
             print FH "#PBS -e run_wrf_$convert_hour.error\n";
             print FH "#PBS -l walltime=00:60:00"."\n";
             print FH "#PBS -A $PROJECT"."\n";
             printf FH "#PBS -l select=1:ncpus=%d:mpiprocs=%d"."\n", $NUM_PROCS, $NUM_PROCS;
             print FH "\n"; #End of PBS commands; add newline for readability
             print FH "mkdir real_rsl\n";
             print FH "mv rsl* real_rsl/\n";
             print FH "mpirun ./wrf.exe\n";
             ...
             if ( ($run_wps =~ /f/i) and ($run_real =~ /f/i) ) {
                $job_feedback = ` qsub < run_wrf.pbs.csh`;
             } else {
                $job_feedback = ` qsub -W depend=afterany:$jobid < run_wrf.pbs.csh`;
             }
             print "$job_feedback\n";
             if ($job_feedback =~ m/(\d+\..*)/) {
                $jobid = $1;
