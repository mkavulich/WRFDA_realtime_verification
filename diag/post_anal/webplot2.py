...
#            self.ax.text(x1-5, y0+5, 'Missing members: %s'%missing_members_string, horizontalalignment='right', transform=None)
    ...
    def plotContour(self):
        data = ndimage.gaussian_filter(self.data['contour'][0], sigma=16)
...
def makeEnsembleList(wrfinit, timerange):
    ...
    for hr in range(shr,ehr+1):
            ...
            #    amem  = '%s/mem%d_%s_all.nc'%(RUN_DIR,mem,yyyymmddhh)
            #    fmem  = '%s/Omem%d_%s_all.nc'%(RUN_DIR,mem,yyyymmddhh)
            #    incr  = '%s/mean_increments.nc'%(RUN_DIR)
	...
	if debug: print file_list[filename] 
        ...
        for array in arrays:
            ...
            if 'arraylevel' in fields[f]:
                if fields[f]['arraylevel'] != 'max': data = fh.variables[array][:,fields[f]['arraylevel'],:,:]
                else: 				     data = np.amax(fh.variables[array][:,:,:,:], axis=1)
...
#            else:                                    data = fh.variables[array][:,0,:,:]
            ...
            if array in ['V10','U10']:  data = data*1.93*10.0 # m/s > .1 kt
            ...
            elif array in ['T2']:                        data = data*1.8 # C->F
            elif array in ['Q2','QVAPOR']:      data = data*1000. # kg->g
            elif array in ['PSFC']:                      data = data/100. # Pa->mb
