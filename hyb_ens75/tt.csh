...
cd ${SCRIPT_DIR}
if ( ! -e ${EXP_DIR_TOP}/${DATE}/FINISHED ) then
   ...
   set tarfile = /glade/scratch/wrfrt/realtime_ensemble/wrfdart_80M40L/output/${DATE}/retro.tar
   ...
   set da_done = false
   while ( $da_done == false )
      if ( -e $EXP_DIR_TOP/${DATE}/wrfvar_output_d01_${DATE} ) then
         set da_done = true
         mv ${SCRIPT_DIR}/job.out ${LOG_DIR}/job_wrfda.${DATE}
      endif
      if ( $da_done == false ) then
         if ( -e ${EXP_DIR_TOP}/${DATE}/FAIL ) then
            if ( $num_rerun < 2 ) then
               \rm -f ${EXP_DIR_TOP}/${DATE}/FAIL
               @ num_rerun ++
...
cd ${SCRIPT_DIR}
set file_to_check = $EXP_DIR_TOP/advance/${DATE}/FINISHED
...
if ( $hh == 00 ) then
   ...
   if ( ! -e ${file_to_check} ) then
      ...
      if ( `head -1 jobid_${DATE} | cut -c1-7` != WARNING ) then
         ...
         set JOB_ID = `cat jobid_${DATE} | cut -c${cind1}-${cind2}`
         if ( $use_standby == true ) then
